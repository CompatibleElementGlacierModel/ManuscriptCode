...
class ISMIP_HOM_A:
    def __init__(self,results_dir):
        for l in [5000,10000,20000,40000,80000,160000]:
            ...
            alpha_ismip = df.Constant(np.deg2rad(0.5))
            ...
            B_exp = S_exp - 1 + 0.5*df.sin(omega_ismip*x)*df.sin(omega_ismip*y)
            B_lin = S_lin - 1
            B_grad_lin = df.diff(B_lin,X)

            beta_exp = df.Constant(10000)
            ...
            with df.CheckpointFile(f'{results_dir}/ismipa-L-{l}.h5', 'w') as afile:
