...
class MMSStatic:
    def __init__(self,results_dir,dry_run=True):
        ...
        for N in [4,8,16,32,64,128]:
            ...
            with df.CheckpointFile(f'{results_dir}/mms_{t_end}_{N}.h5', 'w') as outfile:
                ...
                for M in [1]:
                    ...
                    #model.Ubar0.vector()[:] = 1e-9*np.random.randn(model.Ubar0.vector()[:].shape[0])
                    while count<M:
                        print(t)
                        model.step(t,dt,picard_tol=1e-7,forcing=mms_forcing)
                        t += dt
        ...
        if dry_run==False:
            pickle.dump([self.H_inorms,self.H_2norms,self.U_2norms],open(f'{results_dir}/norms_static.p','wb'))

if __name__=='__main__':
    mms_static = MMSStatic('./results/static/',dry_run=False)
